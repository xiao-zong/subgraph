...
% The rates are presumably optimal.
 ...
 On the other hand, the cumulant method, which uses partition diagrams, enables us to establish qualitative CLT for functionals on RCM. % Find more discussions in Remark~\ref{rem-1}.
...
% \medskip
 ...
 \begin{definition}
   ...
   $\pi_1,\ldots , \pi_n$.
   Let also $\rho \in\Pi(\eta\times[r])$
   be a partition of $\eta\times[r]$. 
 \begin{enumerate} % [\rm 1.]
 \item
 ...
 \item Given $\eta \subset [n]$ 
  ...
  deduced from the
  connected components of the diagram $\rho_G$, as 
 ...
 \begin{proof}
  ...
  Given $\rho \in \mathcal{C} (n+1,r)$ a non-flat partition 
  such that the diagram $\Gamma(\rho,\pi )$ is connected,
  we construct a connected undirected graph
  $\widehat{\rho}$ on $[n+1]$ 
  in which two vertices $i,j\in [n+1]$
  of $\widehat{\rho}$ are connected if and only
  if there exists a block $b\in \rho$ such that $\pi_i \cap b \not= \emptyset$
  and $\pi_j \cap b \not= \emptyset$,
  see Figure~\ref{fig:diagram0-11}-$a)$.  
 ...
 \centering
   \scalebox{0.75}{
 \subcaptionbox{Diagram $\Gamma(\rho,\pi)$ and graph $\widehat{\rho}$.}[.48\textwidth]{%
 \begin{tikzpicture}
 \draw[black, thick] (0,0) rectangle (5,6);
 ...
 \end{tikzpicture}%
     ...
     \scalebox{0.75}{
 \subcaptionbox{Diagram $\Gamma(\rho,\pi)$ and spanning tree $\widebar{\rho}$.}[.5\textwidth]{%
 \begin{tikzpicture}
 \draw[black, thick] (0,0) rectangle (5,6);
 ...
 % In addition, the tree $\widebar{\rho}$ has at most $r$ leaves, because after removing any of root of $\widebar{\rho}$, the remaining partition can be reconnected using no more than $r$ vertices from the root.
 ...
 \begin{proof}
  ...
  % For our analysis of cumulant behavior
  We identify the leading terms in the sum \eqref{cumulant-diagram1}
  over connected partition diagrams, knowing that every
  vertex in $\rho_G$ contributes a factor $\lambda$, and that 
  every edge contributes a factor $c_\lambda$,
  therefore every summand in \eqref{cumulant-diagram1}
  contributes a factor $c_\lambda^{|E(\rho_G)|} \lambda^{|V(\rho_G)|}$. 
 
 % \medskip
 
  Modifying $\rho \in\Pi_{\widehat{1}} ([n]\times[r])$ by splitting a block in two 
   means adding a vertex to $\rho_G$, and therefore a adding factor $\lambda$
   to the corresponding term in \eqref{cumulant-diagram1}.
   At the same time, this entails no loss of edge but possibly 
   the addition of an edge to $\rho_G$, which results into
   an additional factor $c_\lambda$ with $\lambda c_\lambda \gg 1$
   by \eqref{fjnldsf}. 
   Hence, the leading terms in \eqref{cumulant-diagram1} are those associated
   ...
   for a sample of such partition diagram.
 ...
 \end{equation} 
   ...
   In addition, for any connected partition diagram
   $\Gamma(\rho ,\pi )$ with $\rho\in\Pi_{\widehat{1}} ([n]\times[r])$,
   we have 
 ...
 and their contribution is given by a power of order $\lambda^{r-\alpha |E(G)|}$. 
  An example of such partition diagram $\rho$ is given in Figure~\ref{fig:diagram4}, 
  with $G$ a cycle. 
 ...
 \end{tikzpicture}}%
   ...
   \caption{Cycle diagram $\rho_G$ with $G$ a cycle graph and $n=5$, $r=4$.}
 \label{fig:diagram4}
 \end{figure}
 ...
 \noindent
  ...
  the partition diagram~$\rho$ can only be modified
  into a partition diagram~$\sigma$ by splitting a block of $\rho_G$ in two, 
...
Erd{\Horig{o}}s,~P. and R\'enyi,~A.~(1959).
  ...
  \bibitem[\protect\citeauthoryear {Grote and Th{\"a}le}{2018}]{grotethale18}
  ...
  \bibitem[\protect\citeauthoryear {Grote and Th{\"a}le}{2018}]{thale18}
  Grote,~J. and Th{\"a}le,~C.~(2018).
